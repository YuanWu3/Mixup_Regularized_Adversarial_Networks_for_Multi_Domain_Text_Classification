...
def discrepancy(out1, out2):
    return torch.mean(torch.abs(functional.softmax(out1, dim = -1) - functional.softmax(out2, dim = -1)))
        ...
        for i in tqdm(range(num_iter)):
            ...
            for _ in range(n_critic):     
                ...
                for domain in opt.all_domains:
                    ...
                    mixup_unlabeled, d_a, d_b, lam_d = mixup_data(d_inputs, d_targets)
                    mixup_d_outputs = D(F_s(mixup_unlabeled))
                    l_mixup_d = mixup_criterion(criterion, mixup_d_outputs, d_a, d_b, lam_d)
                    # D accuracy
                    _, pred = torch.max(d_outputs, 1)
                    d_total += len(d_inputs)
                    d_correct += (pred==d_targets).sum().item()
                    l_d = functional.nll_loss(d_outputs, d_targets)
                    l_d = l_d + beta * l_mixup_d
                    l_d.backward()
            ...
            for domain in opt.domains:
                ...
                criterion = nn.NLLLoss()
                mixup_input, y_a, y_b, lam = mixup_data(inputs, targets)
                mixup_features = torch.cat((F_s(mixup_input), F_d[domain](mixup_input)), dim = 1)
                mixup_outputs = C(mixup_features)
                mixup_loss1 = mixup_criterion(criterion, mixup_outputs, y_a, y_b, lam)
                #Mixup on unlabeled data
                unlabeled_inputs, _ = utils.endless_get_next_batch(unlabeled_loaders, unlabeled_iters, domain)
                mixup_unlabeled, index1, lam1 = mixup_data1(unlabeled_inputs)
                unlabeled_output = C(torch.cat((F_s(mixup_unlabeled), F_d[domain](mixup_unlabeled)), dim = 1))
                refer_output1 = C(torch.cat((F_s(unlabeled_inputs), F_d[domain](unlabeled_inputs)), dim = 1))
                refer_output2 = C(torch.cat((F_s(unlabeled_inputs[index1, :]), F_d[domain](unlabeled_inputs[index1, :])), dim = 1))
                loss_dis = discrepancy(unlabeled_output, lam1 * refer_output1 + (1 - lam1) * refer_output1)
                l_c = l_c + alpha * mixup_loss1 + rho * loss_dis
